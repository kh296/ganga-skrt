...
using Ganga.  When splitting into subjobs using Ganga's PatientImageSplitter,
the application is limited for each subjob to running only on images with paths
included in those assigned to the subjobs.
"""
...
from skrt.core import filter_on_paths, fullpath, qualified_name
...
class SimpleAlgorithm(Algorithm):
    ...
    def __init__(self, opts={}, name=None, log_level="INFO"):
        ...
        # Define type(s) of image to process
        ...
        # Counter of number of patients analysed
        self.n_patient = 0

        # Counter of number of images analysed
        self.n_image = 0
    ...
    def execute(self, patient=None):
        ...
        for study in patient.get_studies():
            print(f"\nStudy: {study.timestamp}")
            print("Paths to image data for analysis:")
            # If patient has an entry in self.images,
            # only consider images with paths included in this entry.
            # The self.images directory is filled automatically
            # when splitting Ganga jobs based on PatientImageSplitter.
            images = filter_on_paths(study.get_images(self.image_types),
    ...
    def finalise(self):
        ...
        print(f"Number of images analysed = {self.n_image}")
...
if "Ganga" in __name__:
    ...
    # Define and configure the application to be run.
    ganga_app = SkrtApp._impl.from_application(
        get_app(), setup_script, patient_class, patient_opts
    )
    ...
    splitter = PatientImageSplitter(
        images_per_subjob=10,
        separate_patients=True,
        image_types=ganga_app.algs[0].opts["image_types"][0:1],
    )
