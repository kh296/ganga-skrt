...
class PatientImageSplitter(ISplitter):
    ...
    def split(self, job):
        ...
        job_images = self.getJobImages(job, paths)
        ...
        if self.separate_patients:
            ...
            for path, images in job_images.items():
                for idx in range(0, len(images), self.images_per_subjob):
                    subjobs.append(self.newSubjob(
                        job, {path:
                              images[idx : idx + self.images_per_subjob]}))
            ...
            for path, images in job_images.items():
                ...
                while idx1 < len(images):
                    idx2 = min(idx1 + self.images_per_subjob - n_image,
                               len(images))
                    subjob_images.update({path: images[idx1 : idx2]})
                    n_image += (idx2 - idx1)
                    if n_image == self.images_per_subjob:
                        subjobs.append(self.newSubjob(job, subjob_images))
                        subjob_images = {}
                        n_image = 0
                    idx1 = idx2
    ...
    def newSubjob(self, job, subjob_images):
        '''
        Create new subjob.
        ...
        subjob_images_by_id = {Path(path).name: images
                                for path, images in subjob_images.items()}
    ...
    def getJobImages(self, job, paths):
        '''
            ...
            for study in sorted(studies):
                ...
                for image_type in image_types:
                    ...
                    job_images[path].extend(
                            [str(image_path) for image_path
                             in sorted((study / image_type).glob("2*"))])
